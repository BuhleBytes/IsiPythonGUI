/**
 * PublishedQuizzes Component
 *
 * This component displays and manages all quizzes that have been published
 * and are live for students to access and complete. It provides a comprehensive interface
 * for educators to monitor, analyze, and manage their active quizzes with real-time
 * statistics and performance metrics.
 *
 * Key Features:
 * - Display grid of published quizzes with comprehensive metadata
 * - Real-time statistics display (pass rates, attempts, average scores, user engagement)
 * - Advanced search and filtering capabilities (by time limit, title, description)
 * - Multiple sorting options (recent, title, time limit, points, popularity)
 * - Quiz analytics with pass rate color coding
 * - Delete functionality for published quizzes with confirmation
 * - Visual time limit indicators with color-coded styling (Quick/Standard/Extended)
 * - Performance metrics (user attempts, completion rates, average scores)
 ...
 * - Number of users who attempted the quiz
 * - Number of users who passed the quiz
 * - Total attempt count
 * - Average score across all attempts
 * - Publication and due date tracking
 *
 * The component integrates with a custom hook (usePublishedQuizzes) to handle
 * API operations like fetching published quizzes, deletion, and refreshing data.
 *
 * @component
 */
...
export default function PublishedQuizzes({
  ...
  const {
    publishedQuizzes, // Array of published quizzes from API
   ...
   */
  const filteredQuizzes = publishedQuizzes
    .filter((quiz) => {
  ...
  /**
   * Handles viewing a specific quiz
   * Calls the parent component's callback function with the quiz ID
   */
  const handleViewQuiz = (quiz: PublishedQuiz) => {
    if (onViewQuiz) {
      onViewQuiz(quiz.id);
    }
  };
  ...
  /**
   * Handles deleting a published quiz
  ...
  const handleDeleteQuiz = async (
    ...
    // Success and error handling is managed by the usePublishedQuizzes hook
    // The UI will automatically update when the quiz is removed from the list
  };

  /**
   * Loading State Component
   * Displayed while published quizzes are being fetched from the API
   */
  if (loading) {
    return (
   ...
   * Displayed when there's an error fetching or deleting published quizzes
        ...
        <div className="text-center space-y-4 max-w-md">
          ...
          <div className="text-xl font-semibold text-gray-700">
            {error.includes("delete")
              ? "Delete Error"
              : "Failed to Load Published Quizzes"}
          </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        {/* Quiz badges (time category and published status) */}
